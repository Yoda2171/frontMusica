...
export class CartComponent {
  ...
  getCartUser() {
    ...
    console.log(user);
    this.carritoService.getCart(user).subscribe({
      next: (res: any) => {
        console.log(res);
        this.carrito = res;
      },
  ...
  createWebpayTransaction(cartId: number, totalAmount: number) {
    this.carritoService.createWebpayTransaction(cartId, totalAmount).subscribe({
      next: (res: any) => {
        console.log(res);
        this.token = res.token;
        this.urlWebpay = res.url;
        this.router.navigate(['/procesandopago'], {
          queryParams: { token: this.token, url: this.urlWebpay },
        });
      },
      error: (err) => console.log(err),
    });
  ...
  deleteproduct(idcart: number, idproduct: number) {
    this.carritoService.deleteproduct(idcart, idproduct).subscribe({
      next: (res: any) => {
        console.log(res);
        this.getCartUser();
      },
